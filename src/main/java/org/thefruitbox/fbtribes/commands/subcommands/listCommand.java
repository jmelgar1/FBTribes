...
public class listCommand extends SubCommand {
	...
	public void perform(Player p, String[] args) {
		...
		Map<String, Double> sortedList = GeneralUtilities.sortByComparator(tribeManager.generateLeaderboard(), false);
		...
		if(firstEntry <= sortedList.size()) {
			...
			for(Map.Entry<String, Double> entry : sortedList.entrySet()) {
				if(count >= firstEntry && count <= lastEntry) {
					...
					TextComponent tribeInfo = new TextComponent(mainClass.lightGreen.toString() + count + ". " + 
					mainClass.lighterGreen.toString() + tribeName + ChatColor.DARK_PURPLE + " ["
